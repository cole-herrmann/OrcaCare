...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext
{
    ...
    int dir = (self.operation == UINavigationControllerOperationPush) ? -1 : 1;
    ...
    CGRect toVCEndFrame = toVC.view.bounds;
    CGRect fromVCEndFrame = CGRectMake(dir * fromVC.view.frame.size.width, 0, toVC.view.frame.size.width, toVC.view.frame.size.height);
    
    CGRect toVCStartFrame = toVCEndFrame;
    CGFloat width = toVC.view.frame.size.width * ((self.operation == UINavigationControllerOperationPush) ? 1 : -1);
...
//    
//    [transitionContext containerView].backgroundColor = [toVC finalColor];
    
    // First step is required to trigger the load of the visible cells.
    [UIView animateWithDuration:0 delay:0 options:UIViewAnimationOptionCurveLinear animations:nil completion:^(BOOL finished) {
